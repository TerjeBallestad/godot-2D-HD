...
must_haves:
  ...
  artifacts:
    ...
    - path: "assets/textures/shadow_blob.png"
      provides: "Blob shadow texture for Decal projection"
      min_size: 100
  key_links:
...
- `pixel_size = 0.0095` (adjust based on sprite resolution - ~95% of 0.01 for stylized smaller proportions per user decision)
- `offset = Vector2(0, height/2)` where height = sprite pixel height (origin at feet)

**Scale approach (user decision: stylized ~95% proportions):**
- Achieve ~95% scale via `pixel_size = 0.0095` instead of the standard 0.01
- This makes the sprite slightly smaller relative to the 3D world
- Do NOT use modulate for scale (modulate affects color/alpha only)
- Do NOT use transform.scale on Sprite3D (distorts pixel art)
...
5. Verify Sprite3D pixel_size is 0.0095 (not 0.01)
6. Verify assets/textures/shadow_blob.png exists
  </verify>
  <done>
PlayerCharacter scene exists with properly configured Sprite3D (Y-axis billboard, nearest filtering, 0.0095 pixel_size for ~95% scale), blob shadow system (ShapeCast3D + Decal), and collision shape.
  </done>
</task>
...
4. Adjust `Pixel Size` if character appears too large/small (try 0.008-0.015, default is 0.0095 for ~95% scale)
...
2. Verify these visual requirements:
   ...
   - [ ] Character faces the camera horizontally (Y-axis billboard rotates horizontally only)
   - [ ] Pixels are crisp (no blurring or interpolation)
   - [ ] Blob shadow is visible on floor beneath character
   - [ ] Character scale looks appropriate relative to furniture (~95% of realistic, slightly stylized)
   - [ ] Character feet appear to touch the floor

**Step 4: Orbit camera test (if controls exist)**
If you can rotate the camera view in editor, verify the billboard follows horizontally while staying upright.
  </how-to-verify>
  <resume-signal>
...
3. Sprite3D has pixel_size=0.0095 for ~95% scale (user decision)
4. Blob shadow Decal projects onto floor
5. Character instanced in interior_scene.tscn
6. User visually approves the HD-2D character integration
</verification>
...
- Sprite billboards horizontally to face camera (Y-axis only, stays upright)
- Sprite displays with crisp nearest-neighbor filtering (no blur)
- Sprite integrates visually with 3D environment (~95% stylized scale, feet on floor)
